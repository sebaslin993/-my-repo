 ...
 *   Left button press   → TouchEventDown  (triangle turns RED)
 *   Mouse move (held)   → TouchEventMove  (triangle turns GREEN)
 *   Left button release → TouchEventRelease (triangle turns BLUE)
 *
 * Ported from OpenGL ES 3.0 Cookbook – Chapter 2, Touch Events.
 */
...
int main()
{
    LOGI("Quiz 3 Fan Dashboard - WebGL / Emscripten (GLFW)");

    if (!glfwInit()) { LOGE("glfwInit failed"); return -1; }

    // Request WebGL 2.0 context (GLES 3.0)
    glfwWindowHint(GLFW_CLIENT_API,            GLFW_OPENGL_ES_API);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 0);

    g_window = glfwCreateWindow(800, 600, "Quiz 3 Fan Dashboard", nullptr, nullptr);
...
int main()
{
    LOGI("Quiz 3 Fan Dashboard - WebGL / Emscripten (SDL2)");
...
// GLFW callbacks store state for touch simulation via mouse
static bool  g_mouseDown = false;
...
int main()
{
    std::cout << "Quiz 3 Fan Dashboard - Desktop (GLFW)\n";
    std::cout << "HUD controls speed and zoom; click fan parts to highlight them.\n";
    ...
    // Request an OpenGL ES 3.0 context via GLFW
    glfwWindowHint(GLFW_CLIENT_API,             GLFW_OPENGL_ES_API);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR,  3);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR,  0);
    glfwWindowHint(GLFW_OPENGL_PROFILE,         GLFW_OPENGL_ANY_PROFILE);

    GLFWwindow* window = glfwCreateWindow(800, 600, "Quiz 3 Fan Dashboard", nullptr, nullptr);
    if (!window) {
